...
void nora_desktop_retile(struct nora_desktop *desktop) {
  ...
  // step 1. configure width, height, x and y for the focused view.
  {
    ...
    wlr_scene_node_set_position(
        &desktop->focused_view->xdg_toplevel.scene_tree->node, view_x, view_y);

    int view_width = (num_views == 0) ? area->width - (2 * border_width)
                                      : (area->width / 2) - border_width;
    int view_height = area->height - (2 * border_width);

    wlr_xdg_toplevel_set_size(desktop->focused_view->xdg_toplevel.xdg_toplevel,
  ...
  if (num_views >= 1) {
    ...
    wl_list_for_each(view, &desktop->views, link) {
      ...
      wlr_log(WLR_INFO, "tiling window (%s) at (%d, %d) with (%dx%d)",
              view->xdg_toplevel.xdg_toplevel->title, view_x, view_y,
              view_width, view_height);
      wlr_scene_node_set_position(&view->xdg_toplevel.scene_tree->node, view_x,
                                  view_y);
      wlr_xdg_toplevel_set_size(view->xdg_toplevel.xdg_toplevel, view_width,
                                view_height);
